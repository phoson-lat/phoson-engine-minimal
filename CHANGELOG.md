...
- **mcp**: forward session/display env vars to stdio MCP servers
...
- **llm**: forward a per-request `think` toggle to Ollama for hybrid reasoning
  models (e.g. Qwen3). `ModelConfig.think` wins; when unset, the
  `PHOSON_OLLAMA_THINK` env var is the fallback. Leaving both unset keeps the
  model's default thinking behaviour untouched, so models without the toggle
  are never affected.
- **installer**: add a per-user Windows PowerShell installation.
...
- **llm**: OmniRoute as an OpenAI-compatible provider — local gateway
  default `http://localhost:20128/v1`, optional API key, configurable
  base URL in the setup wizard, live `/model` listing (#220)
...
- **cli**: tint monitor-wake turns + human-readable subagent metrics (#207)
...
- **otel**: wire phoson_plugin_otel into the CLI (opt-in enable_otel) (#140)
- **otel**: phoson_plugin_otel — run→step→llm/tool span tree + local trace-file sink (#140)

## v0.26.3 (2026-09-05)

> First published release of this line: **v0.26.2 was never released**
> (no tag, no PyPI), so the fixes listed under v0.26.2 — including the
> async SSRF-hook fix — ride in this version.
