...
- `phoson_plugin_otel`: traces every agent run (main engine and sub-agents) as an OTel span tree (`run → step → llm_call`/`tool_call`) and exports it in OTLP/HTTP JSON. Slice 1 ships the local trace-file sink (`enable_otel = true`, opt-in); a minimal OTLP/HTTP collector POST is included for the real-exporter path. No `opentelemetry-*` dependency. See `phoson_plugin_otel/README.md`.
