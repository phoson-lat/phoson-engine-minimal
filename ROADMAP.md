...
> **Actualizado:** 2026-09-05 · estado de referencia **v0.28.0** (v0.28.0: #140 slice 2 (export OTLP/HTTP real a colector + oráculo de conformidad contra `opentelemetry-proto`) + #139 (eval set de agente 6→15 tasks + gate de no-regresión nightly con bootstrap self-seed + split held-out) — PR #203, `fdcaf17` + PR #204, `fc953e7`; v0.27.0: #140 slice 1 `phoson_plugin_otel` (trazas OTel run → step → llm/tool a trace-file JSON local, opt-in `enable_otel`) — PR #202, `5f6dccd`; v0.26.3: #181 tools nativos `grep` + `glob` — PR #201, `0737f7c`; v0.26.2 — *nunca publicada*, los fixes van dentro de v0.26.3: #182 `@import` confinado al repo + #183 `web_fetch` SSRF/limite de descarga (+ hook async) + #184 system prompt del sub-agente + `agents` no ofrecido + #185 cinco fixes CLI + #200 plugin spec degradado a warning — PR #196, `7d5cf6c` + PR #199, `602217e` + PR #198, `5a99f91` + PR #197, `ced2d51` + PR #200, `25a2bb8`; v0.26.1: #178 `stop_reason` normalizado + truncación/enforcement + `result.truncated` y #177 `RetryingChat` en `build_chat` + `RetryMiddleware` deprecado — PR #195, `2507df0` + PR #194, `6080ba0`; v0.26.0: #179 `patch_file` unicidad + CRLF + #180 `cat -n`/caps/descripciones/system prompt ACI — PR #193, `67368b0`; v0.25.1: #175 `is_simple_shell_command` + `match_args` obligatorio — PR #192, `af502f0`) · 2365 tests pasados · ruff limpio (pyright: 1 error preexistente en `gemini.py`).
>
...
| Harness infra (H-1/H-2) | ✅ **H-2 complete** y **H-1 shipped v0.28.0**. #140: slice 1 ✅ v0.27.0 (PR #202, `5f6dccd`) — `phoson_plugin_otel`, trazas OTel run → step → llm_call/tool_call a trace-file JSON; slice 2 ✅ v0.28.0 (PR #203, `fdcaf17`) — export OTLP/HTTP real a colector (body puro `ExportTraceServiceRequest`, headers) + oráculo de conformidad contra `opentelemetry-proto`. #139 ✅ v0.28.0 (PR #204, `fc953e7`) — eval set de agente 6→15 tasks (oráculo `SOLVE` model-free en CI) + gate de no-regresión nightly (baseline self-seed, ruido ≥3 corridas, split held-out, empates rechazados). H-1/H-2 cerrados. |
...
| ACI (edit/search/prompt) | ✅ #181 (grep/glob) shipped v0.26.3 (PR #201, `0737f7c`): tools nativos read-only (rg si existe + fallback Python, `.gitignore`, caps), wiring de registry/permisos/prompt/TUI + 2 bench tasks de búsqueda. ✅ #179 + #180 shipped v0.26.0 (PR #193, `67368b0`): unicidad de `patch_file` + CRLF + `cat -n` + caps + descripciones + secciones ACI del system prompt. Medición H-1 (#139) pendiente. |
...
| [#181](https://github.com/phoson-lat/phoson-engine-minimal/issues/181) | Tools nativas `grep` y `glob` | F-21b | 🟠 | M | ✅ **cerrado** | Shipped v0.26.3 (PR #201, `0737f7c`): `phoson_cli/tools/search.py` — `grep` (regex, `rg --json` + fallback Python idéntico, `path:line: content`) y `glob` (mtime desc, 500) read-only, `.gitignore` (parser propio, funciona sin git) + noise dirs, caps (100/1000 líneas, 16 KB, 2 MB/file, 20k files). Registry, `MATCH_ARGS` (`path`), system prompt (reemplaza el hint "no native search"), TUI (🔍/🗂), docs. CI instala ripgrep (paridad rg↔Python); sin `rg` se ejercita el fallback vía tests públicos. 2 bench tasks de búsqueda (`find-call-sites`, `locate-definition`). 51 tests. **Pendiente:** medición contra H-1 (#139, Sprint C). |
| [#140](https://github.com/phoson-lat/phoson-engine-minimal/issues/140) | `phoson_plugin_otel` (trazas) | H-2 | 🟢 | M | ✅ **cerrado** | **Slice 1** shipped v0.27.0 (PR #202, `5f6dccd`): middleware OTel (run → step → llm_call/tool_call), sink trace-file JSON OTLP/HTTP (stdlib-only, opt-in `enable_otel`), sub-agentes aislados por `ContextVar`, flush en cleanup. 98 tests. **Slice 2** shipped v0.28.0 (PR #203, `fdcaf17`): export OTLP/HTTP real a colector — `OtlpHttpSink` POSTea el body puro `ExportTraceServiceRequest` a `<endpoint>/v1/traces` (descartó el envelope `phoson_trace`, que no round-trip), key `headers` + `_parse_otlp_headers()` (formato estándar `OTEL_EXPORTER_OTLP_HEADERS` `k=v,k2=v2`), `build_request()` síncrono para testear el wire format. Oráculo de conformidad contra `opentelemetry-proto` (test-only dev dep; runtime sigue stdlib-only). 15 tests nuevos. |
| [#139](https://github.com/phoson-lat/phoson-engine-minimal/issues/139) | Eval set 15–25 tareas + gate nightly | H-1 | 🟢 | M | ✅ **cerrado** | Shipped v0.28.0 (PR #204, `fc953e7`): eval set 6→15 tasks deterministas (una por eje: creación, transform, búsqueda, conteo a escala, fix root-cause, edición multi-archivo, alógica, parse). Cada task lleva `SOLVE` de referencia co-located → CI afirma `setup→SOLVE→check` (oráculo de conformidad **model-free**). `bench/baseline.py`: gate + baseline puro (noise = std de rates por corrida, **pass solo si current > baseline − noise**, empates rechazados, per-task deltas = contrato falsable). `baseline.json` sentinel `pass_rate:null` → primera corrida real self-seed (sin números inventados); `--bootstrap` re-seed. Split held-out (`heldout.txt` + `--no-heldout`) reportado aparte. Workflow nightly (modelo local fijo Ollama, ≥3 corridas, publica, falla bajo baseline−noise). 36 tests nuevos; suite 2365 passed. |
...
1. #181   grep + glob nativos.                                                  M
          ✅ shipped v0.26.3 (PR #201, `0737f7c`):
...
2. #140   slice 1: trace-file JSON por run (run → step → tool_call).            M
          ✅ shipped v0.27.0 (PR #202, `5f6dccd`):
          `phoson_plugin_otel` — middleware OTel (run → step →
          llm_call/tool_call), sink JSON OTLP/HTTP local (stdlib-only,
          opt-in `enable_otel`), sub-agentes aislados por ContextVar,
          flush en cleanup; 98 tests.
          slice 2: export OTLP/HTTP real a colector.                        M
          ✅ shipped v0.28.0 (PR #203, `fdcaf17`):
          `OtlpHttpSink` POSTea el body puro ExportTraceServiceRequest a
          /v1/traces (descartó el envelope phoson_trace), headers vía
          OTEL_EXPORTER_OTLP_HEADERS, build_request() síncrono; oráculo de
          conformidad vs opentelemetry-proto (dev-only, runtime stdlib).
          15 tests.
3. #139   bench/ a 15–25 tareas; baseline ≥3 corridas con modelo local fijo
          ...
          ✅ shipped v0.28.0 (PR #204, `fc953e7`): eval set 6→15 tasks
          (oráculo SOLVE model-free en CI); gate/baseline/varianza/split en
          bench/baseline.py puro (empates rechazados); baseline self-seed
          (sentinel pass_rate:null) + --bootstrap; held-out reportado aparte;
          workflow nightly (Ollama fijo, ≥3 corridas). 36 tests; 2365 passed.
