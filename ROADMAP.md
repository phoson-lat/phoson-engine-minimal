...
| Harness infra (H-1/H-2) | #140 slice 1 ✅ shipped v0.27.0 (PR #202, `5f6dccd`): `phoson_plugin_otel` — trazas OTel run → step → llm_call/tool_call a trace-file JSON (sink OTLP/HTTP JSON local, opt-in). #139 (eval set + gate nightly) pendiente — ahora con los datos de traza que desbloquea #140. |
...
| [#140](https://github.com/phoson-lat/phoson-engine-minimal/issues/140) | `phoson_plugin_otel` (trazas) | H-2 | 🟠 | M | **C** (slice 1 ✅) | Slice 1 shipped v0.27.0 (PR #202, `5f6dccd`): `phoson_plugin_otel` — middleware OTel (run → step → llm_call/tool_call), sink trace-file JSON OTLP/HTTP (stdlib-only, opt-in `enable_otel`), sub-agentes aislados por `ContextVar`, flush en cleanup. 98 tests. **Pendiente:** slice 2 export OTLP real a colector. |
...
2. #140   slice 1: trace-file JSON por run (run → step → tool_call).            M
          ...
          flush en cleanup; 98 tests. Slice 2 (export OTLP real)
          pendiente.
