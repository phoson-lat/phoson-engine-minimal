...
def test_custom_max_tokens_key():
    chat = OpenAICompatibleChat(
        base_url=DEFAULT_URL, api_key="test-key", max_tokens_key="max_completion_tokens"
    )
...
def test_default_headers_are_passed():
    chat = OpenAICompatibleChat(
        base_url=DEFAULT_URL, api_key="test-key", default_headers={"X-Custom": "value"}
    )
...
def test_repr_includes_provider_name():
    chat = OpenAICompatibleChat(
        base_url=DEFAULT_URL, api_key="test-key", provider_name="MyProvider"
    )
