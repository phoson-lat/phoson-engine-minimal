...
async def _settle(app: PhosonTextualApp, pilot, pauses: int = 10) -> None:
    """Let the sink's FIFO queue drain (widget mounts are async)."""
...
async def test_composer_disabled_while_running(tmp_path: Path) -> None:
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        app._controller.engine.stream = slow_stream.__get__(app._controller.engine)
        with _patch_models():
            # Submit synchronously: the run task exists the moment
            # submit_composer() returns. (A pilot.pause() in between is
            # flaky — it can drain the whole 1s stream and end the turn
