...
def test_shell_caps_redraw_frequency_with_min_redraw_interval(app: PhosonApp) -> None:
    """I-84: the Application sets a hard floor on repaint frequency so
    redundant invalidations (ticker + streaming) coalesce into fewer full
    layout/ANSI passes. Must stay below one human-spaced keystroke/scroll
    tick (~100 ms) to keep navigation latency imperceptible."""
    assert app.app.min_redraw_interval is not None
    assert 0.03 <= app.app.min_redraw_interval <= 0.06
