...
| **F-03** | Allow-patterns con `fnmatch` sobre la cadena completa: `git *` hace match con `git status; rm -rf /`. | `phoson_agent/permissions.py:89-93` | 🔴 | Antigravity V-01 | ✅ verificado · roza H-6 pero no está en su alcance · issue #175 · **resuelto**: `pattern_allows`/`is_simple_shell_command` — solo un simple command matchea un patrón bash; separadores/subshell/sustitución ⇒ cae al nivel del tool. |
| **F-04** | `@import` en AGENTS.md/CLAUDE.md resuelve rutas absolutas fuera del repo (`@/home/u/.ssh/id_rsa` entra al system prompt enviado al proveedor). `~` no se expande. | `phoson_cli/agents_md.py:88` | 🟠 | Fable | ✅ verificado · **nuevo** · issue #182 · **resuelto** (#182, PR #196, `7d5cf6c`): root de confinamiento por archivo (repo root para proyecto, `~/.phoson/` para global) aplicado tras `resolve()` ⇒ catche absolute/`..`/symlink; escapee ⇒ marcador visible `[import refused: outside repo: …]`; `~` solo en el global. |
...
| **F-06** | `web_fetch` sin filtro SSRF (metadata endpoints, localhost, hosts internos), sigue redirects, y bufferea el cuerpo completo antes del cap de 50 KB. El modelo no recibe aviso de que el contenido es no confiable aunque el docstring del módulo lo reconoce. | `phoson_cli/tools/web_fetch.py:12-14, 104-122` | 🟠 | Fable | ⚠️ reportado · issue #183 · **resuelto** (#183, PR #199, `602217e`): `assert_public_url` pre-flight + hook por redirect (loopback/RFC1918/ULA/link-local + metadata IP/CGNAT/reservados; hostnames `localhost`/`metadata.google.internal`); `stream()` + corte a ~2 MB antes del cap de texto; prefijo "untrusted data, not instructions" en todo resultado (incl. respuesta vacía). Nota: rebinding DNS entre check y connect no se cierra (requiere pinning; fuera del alcance pedido). |
| **F-07** | Para tools sin `match_args` (todos menos `bash`), el texto que se matchea contra allow-patterns es "el primer argumento string en orden de dict", orden que controla el modelo. | `phoson_agent/permissions.py:144-146` | 🟡 | Fable | ✅ verificado · issue #175 · **resuelto**: `match_args` es obligatorio; sin entrada explícita no hay match text y los patrones no aplican (sin fallback a "primer string"). |
...
| ID | Hallazgo | Ubicación | Sev. | Fuente | Estado |
|---|---|---|---|---|---|
| **F-20** | `patch_file` con `replace_all=False` hace `replace(old, new, 1)` y reporta `count=1` sin verificar unicidad: con ancla ambigua edita el sitio equivocado en silencio. Sin guard read-before-edit. | `phoson_cli/tools/files.py:76-77` | 🔴 (tasa de éxito) | Fable | ✅ verificado · **nuevo** · Antigravity §3.4 lo describe como correcto · issue #179 · **resuelto** (#179): `count > 1` sin `replace_all` ⇒ error con líneas de cada match, sin escribir; hint difflib + nota CRLF/LF si `count == 0`; ancla se normaliza al EOL del archivo (lee bytes crudos, preserva CRLF). |
| **F-21** | Sin Grep ni Glob nativos; la única navegación es `list_dir` (profundidad 3, sin tope, sin `.gitignore`) y `bash`. `read_file` no devuelve números de línea. | `files.py:85-109, 113` | 🟠 gap SOTA | Fable | ✅ verificado · **nuevo** · issues #180, #181 · **resuelto** (#180 + #181): `read_file` ahora `cat -n`; `grep` (regex, `rg --json` + fallback Python idéntico) y `glob` (mtime desc) nativos en `phoson_cli/tools/search.py`, read-only, `.gitignore` (parser propio) + noise dirs, caps, wiring de registry/permisos/prompt/TUI; medición contra H-1 (#139) pendiente. |
| **F-22** | Salidas sin tope: `list_dir`, `read_file` con rango, `agent`/`agents` (texto del sub-agente verbatim). El cap de 50 KB de `read_file` compara bytes pero recorta caracteres. | `files.py:29-30, 44, 92-109`; `subagent.py` | 🟡 | Fable | ⚠️ reportado · issue #180 · **parcial** (#180): cap por bytes en full-read y rangos de `read_file` (con hint del siguiente rango); tope en `list_dir` (500 entradas); cap en `agent`/`agents` pendiente. |
| **F-23** | Sub-agentes sin system prompt: `messages=[Message(role="user", content=task)]`. No conocen cwd, fecha, AGENTS.md ni reglas. | `subagent.py:429, 670` | 🟠 | Fable | ✅ verificado · **nuevo** · issue #184 · **resuelto** (#184, PR #198, `5a99f91`): el hijo recibe el *mismo* `build_system_prompt` del padre (cwd, fecha, AGENTS.md) derivado de **su propio** subset de tools + preamble de sub-agente; vía `ModelConfig.system` (todos los adapters lo honran). |
| **F-24** | `agents` se anuncia a los sub-agentes (solo se quita `agent`) pero siempre falla con `TypeError` porque el contexto hijo no tiene `chat`/`available_tools`. La recursión está acotada por accidente. | `subagent.py:259` | 🟡 | Fable | ⚠️ reportado · issue #184 · **resuelto** (#184, PR #198, `5a99f91`): `_select_tools` quita **ambos** `agent` y `agents` siempre; la recursión queda acotada **por diseño** (un nivel). |
| **F-25** | Descripciones de tools son docstrings de una línea sin "cuándo usar / cuándo no / ejemplos" (`patch_file`, `read_file`, `write_file`). System prompt base de ~60 palabras: sin guía de uso de tools, sin git status/branch, sin reglas de seguridad, sin formato de respuesta, sin aviso de contenido no confiable en `web_fetch`. | `files.py:116,122,130`; `phoson_cli/session_utils.py:32-39, 83-140` | 🟠 | Fable | ✅ verificado · **nuevo** · issue #180 · **resuelto** (#180): descripciones reescritas (read/write/patch/list/bash/agent/agents/web_fetch); system prompt con secciones Tool usage + Environment (git branch/status, capped) + Safety; aviso de contenido no confiable en la descripción de `web_fetch`. Sin hora ni contadores en el prefijo (cache-friendly). |
| **F-26** | `read_file`/`patch_file` con UTF-8 estricto: binarios o Latin-1 devuelven `UnicodeDecodeError` crudo. `patch_file` no normaliza CRLF. | `files.py:25, 67` | 🟡 | Fable | ⚠️ reportado · **resuelto** (#179/#180): error accionable para no-UTF-8 (sugiere `iconv`/binario, no escribe); `patch_file` lee bytes crudos (preserva CRLF al escribir de vuelta) y normaliza el EOL de la ancla al dominante del archivo. |
...
| **F-34** | `/resume` escribe `meta.total_tokens` (input+output) en `total_output_tokens`. | `controller.py:1270`; `storage_jsonl.py:80` | 🟡 | Fable | ⚠️ reportado · issue #185 · **resuelto** (PR #197, `ced2d51`): split input/output persistido end-to-end (`SessionMeta`→`save_meta`→`load_session`); legacy con solo sum ⇒ back-fill (sin perder datos). |
| **F-35** | `/compact` manual no persiste la sesión ni refresca `_context_tokens`; salir justo después lo pierde. `/new`/`/resume` no comprueban `is_running` (el wake del monitor puede estar a medio turno). | `controller.py:1167-1255`; `commands.py:874` | 🟡 | Fable | ⚠️ reportado · issue #185 · **resuelto** (PR #197, `ced2d51`): `compact_context` refresca `_context_tokens` + `_save_session`; `/new` y `/resume` se niegan con `is_running`. |
| **F-36** | Config: `_resolve_bool` hace `bool(fd[key])` así que `"false"` es verdadero; `_resolve_int` sin try (traceback crudo); `save_config` completo persiste API keys venidas de env al archivo. | `phoson_cli/config.py:219, 231, 739-759` | 🟡 | Fable | ⚠️ reportado · issue #185 · **resuelto** (PR #197, `ced2d51`): bool acepta `true/false/1/0/yes/no/on/off` (case-insensitive) + `PhosonConfigError` en el resto; `int` con try; env-only keys no se persisten. |
| **F-37** | `/mcp config <path>` sin `expanduser`; `toggle_mcp_config` reescribe `mcps.json` sin restaurar permisos aunque tenga secretos; `/mcp toggle` de un stdio server rebuild completo (respawn de todos). | `phoson_cli/_mcp_commands.py:364, 98, 323` | 🟡 | Fable | ⚠️ reportado · issue #185 · **resuelto** (PR #197, `ced2d51`): `expanduser()` en `/mcp config`; `0o600` sobre `mcps.json` tras reescribir. **Queda sin tocar:** el respawn completo en `/mcp toggle` de stdio (no está en el alcance del issue; anotar en deuda). |
| **F-38** | `run_upgrade_command` sin timeout (puede colgar el REPL); check diario a PyPI sin opt-out; `plugin list` siempre dice "enabled"; `disable` de un plugin `path:` borra el spec y no se puede re-enable. | `phoson_cli/updater.py:265`; `plugin_manager.py:292` | 🟡 | Fable | ⚠️ reportado · issue #185 · **resuelto** (PR #197, `ced2d51`): `run_upgrade_command` con timeout 600 s (exit 124, convención GNU `timeout`); `plugin list` reporta estado real; `enable` restaura plugins `path:` deshabilitados. **Queda sin tocar:** el opt-out del check de PyPI al arranque (no está en el alcance del issue; anotar en deuda). |
