...
**Decisión final: el fix barato que ya se apuntaba arriba.** El único bypass universal es una *feature del terminal*, no de la app: mantener `Shift` mientras se hace drag le dice al terminal que ignore el mouse-tracking de la app para ese gesto y haga su propia selección nativa (funciona en GNOME Terminal, iTerm2, Alacritty, WezTerm, Ghostty, kitty, Windows Terminal). Se añadió `[Shift+Drag] Select text` al footer (antes solo vivía enterrado en un comentario de código) y una sección en el README que explica la causa raíz — cero código nuevo en `app.py` más allá de la cadena del hint, cero superficie de bugs, y comunica la limitación real en vez de aparentar resolverla.

### G4 — Hyperlinks clicables ([#58](https://github.com/phoson-lat/phoson-engine-minimal/issues/58)) ✅ v0.13.3
El spike confirmó la segunda vía apuntada aquí: reinyectar OSC 8 post-procesando el ANSI, no un fragmento custom. `formatting.py` pasaba `hyperlinks=False` a `Markdown(...)` porque el parser `ANSI()` de prompt_toolkit solo entiende CSI/SGR (`\x1b[...m`) — una secuencia OSC 8 (`\x1b]8;;URL\x1b\\`) que no arranca con `[` cae en su rama de "ignorar" carácter por carácter, y sus bytes crudos (`8;id=...;https://...`) terminaban como texto literal alrededor del link.

`Markdown(..., hyperlinks=True)` vuelve a estar activo en los tres call-sites (`render_streaming_panel`, `render_history` ×2 en `formatting.py`, y el fallback no-streamed de `renderer.py`). El REPL clásico imprime directo a un `Console` real (sin re-parseo intermedio), así que ahí no necesitaba nada más. El full-screen sí, porque su bridge Rich→ANSI pasa por `ANSI()` antes de llegar al renderer de prompt_toolkit — y ahí funciona el mecanismo que prompt_toolkit ya documenta para exactamente este caso: texto entre `\001` (SOH) y `\002` (STX) se parsea como fragmento `"[ZeroWidthEscape]"`, y el renderer lo escribe con `output.write_raw()` — sin contar ancho, sin escapar nada, tal cual. Nuevo módulo puro `phoson_cli/hyperlinks.py::osc8_passthrough` envuelve cada secuencia OSC 8 en `\001...\002` antes del wrap en `ANSI(...)`; se aplica dentro de `BlockAnsiCache.get_or_render` (`fullscreen/render.py`), una vez por bloque cacheado por ancho — mismo costo que el propio render, cero impacto en el hot path del streaming (que sigue en texto plano hasta congelarse en un bloque).
...
3. **Cada ítem termina con test + entrada en CHANGELOG** — la disciplina actual (1358 tests) es la mayor fortaleza del repo; no erosionarla por velocidad.
