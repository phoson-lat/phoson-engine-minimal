...
| [G3](https://github.com/phoson-lat/phoson-engine-minimal/issues/57) | Seleccionar/copiar texto del chat (full-screen) — modo de copia por teclado | **P2** | S-M | 🟠 Medio | [#57](https://github.com/phoson-lat/phoson-engine-minimal/issues/57) | ✅ v0.13.2 |
...
En full-screen, `mouse_support=True` (para la rueda del chat) captura el mouse y quita la selección nativa click-drag del terminal. Se implementa el **modo de copia por teclado** (la solución terminal-agnostic; el workaround *Shift+drag* sigue funcionando donde el emulador lo soporta — GNOME Terminal, iTerm2, Alacritty — y queda documentado). `F2` (nueva acción `copy_mode`, remapeable/desligable en `[keys]` como cualquier otra) o `/copy` entran en el modo: el **ancla** se fija en la celda superior izquierda del pane visible y el **cursor** arranca en la inferior derecha, de modo que la página visible entera queda preseleccionada (copiar "lo que veo" es un solo Enter). Flechas mueven el extremo que se extiende (←/→ envuelven entre fin/inicio de línea; ↑/↓ conservan la columna), `Home`/`End` saltan al inicio/fin de la fila, `PgUp`/`PgDn` saltan una página completa. La selección se pinta como una banda de *reverse video* (`\x1b[7m`) sobre el ANSI cacheado del chat (recalcado solo en el modo activo, sin tocar la caché de bloques) y el footer cambia a las pistas del modo. `Enter` (o `Ctrl+Y`) **yanka** el rango al clipboard del sistema — `wl-copy`/`xclip`/`pbcopy`, el inverso del read de Ctrl+V — y muestra los caracteres copiados; sin herramienta de clipboard (p. ej. SSH sin `xclip`) avisa por qué no se copió. `Esc` cancela. **Ruteo de teclas:** al activar el modo se apagan los base-bindings (condición `_active_float is None and not _copy_active`), el foco se mueve al *chat window* (que no es focusable por defecto pero sí es enfocable como `Window`), y un set de keys del app vía `DynamicKeyBindings` (activo solo con `_copy_active`) toma el control de flechas/Enter/Esc/Ctrl+Y — así el buffer del composer ya no puede interceptarlas, y tipar en modo normal sigue intacto (el binding `F2` vive en el app, no en el composer). Nuevo módulo puro `copy_range.py` (matemática de rango: `Pos`, `range_text`, `selection_line_span`, `step_page`, `plain_lines`, `apply_reverse_highlight`) y el lado write en `clipboard.py` (`write_clipboard_text` + `clipboard_write_available`/`hint`). Tests: `test_g3_copy_mode_unit.py` (35) + e2e de teclas reales vía PipeInput (F2 → flechas → Enter copia y sale; F2 → Esc cancela; el tipado no se desvía).
