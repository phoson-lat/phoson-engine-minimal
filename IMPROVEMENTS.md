...
| [G3](https://github.com/phoson-lat/phoson-engine-minimal/issues/57) | Seleccionar/copiar texto del chat (full-screen) — selección con mouse + modo de copia por teclado | **P2** | S-M | 🟠 Medio | [#57](https://github.com/phoson-lat/phoson-engine-minimal/issues/57) | ✅ v0.13.2 |
...
### G2 — Prompt caching ([#69](https://github.com/phoson-lat/phoson-engine-minimal/issues/69)) ✅ v0.13.1
El engine reenvía el prompt completo en cada request. Se añade soporte de prompt caching en los dos providers donde hace mayor impacto, y se expone el uso de caché en las métricas de sesión (`TokenUsage.cache_read`/`cache_write`, acumulados en `SessionMetrics`, visibles en `/status` como `cache  R read / W write` y en `/tokens` como `cache=Rr/Ww`):

- **Prefijo estable (prerrequisito):** el system prompt es el prefijo del que dependen las cachés de todos los providers, pero llevaba `Current time is YYYY-MM-DD HH:MM:SS` (segundos) — cambiaba cada request y rompía cualquier caché. Ahora es `Current date is YYYY-MM-DD` (timezone intacto, B1): constante por día completo; para la hora exacta el modelo puede usar `date` (bash siempre está en el registro). Tests de regresión G2.
- **Anthropic (directo):** breakpoints `cache_control: {type: ephemeral}` (TTL 5 min; los 1 h duplican el precio de escritura y no aportan en tráfico agéntico turno a turno) en los 3 de los 4 slots permitidos: system prompt (ahora bloque-list), última tool definition, y último bloque de la última message (avance con el historial — en un loop ReAct el último mensaje suele ser `tool_result` de user, que sí acepta marcador; `tool_use` no). El uso (`cache_creation`/`cache_read`) ya se parseaba; ahora la caché se activa de verdad y el costo usa los precios de caché.
- **OpenRouter:** `ModelConfig.session_id` (nuevo) → `session_id` en el body = **sticky routing** (el provider se fija por conversación: caché warm desde el primer turno y sin cambio silencioso de provider a mitad de sesión); `anthropic/*` además opta a la caché automática con `cache_control` top-level (OpenRouter lo traduce a Bedrock/Vertex); modelos con caché implícita (OpenAI/DeepSeek/Gemini 2.5+) no necesitan nada. El loop compartido ahora parsea también `prompt_tokens_details.cache_write_tokens` (y el callback de costo de OpenAI lo pasa a `calculate_cost` para los precios de escritura de GPT-5.6+). Atribución de app por defecto: `HTTP-Referer: https://phoson.lat`, `X-OpenRouter-Title: phoson-cli`, `X-OpenRouter-Categories: cli-agent` (sobrescribibles).
- El controller pasa `tree.session_id` al `ModelConfig` de cada run (los subagents no — comparten el model pero no la conversación).
...
### G3 — Seleccionar/copiar texto del chat ([#57](https://github.com/phoson-lat/phoson-engine-minimal/issues/57)) ✅ v0.13.2
En full-screen, `mouse_support=True` (para la rueda del chat) captura el mouse y quita la selección nativa click-drag del terminal. Se implementa la **selección con mouse** (el gesto natural: click-drag sobre el chat) **y el modo de copia por teclado** (fallback terminal-agnostic; el workaround *Shift+drag* también sigue funcionando donde el emulador lo soporta — GNOME Terminal, iTerm2, Alacritty — y queda documentado).

**Selección con mouse (gesto por defecto).** El handler de mouse vive en el *control* del chat, no en el `Window`: prompt_toolkit convierte la coordenada de pantalla en `(row, col)` del contenido vía `rowcol_to_yx` **antes** de llamarlo, y `row` es la fila *global* del transcript (ya ajustada al scroll actual — un click en la primera fila visible con scroll S reporta la fila S), el mismo espacio en el que trabaja `copy_range`. Así: un **press izquierdo** entra en el modo de copia anclado en la celda pulsada (nada preseleccionado), el **drag** extiende la selección bajo el puntero (reverse video), y el **release** sobre un rango real **yanka** el texto al clipboard del sistema y deja la selección visible para refinarla con teclado y re-copiar con `Enter`. Un **click simple** (sin drag) sale del modo y refoca el composer; un **doble-click** (2 releases en la misma celda en <0.3 s, el mismo umbral de `BufferControl` de prompt_toolkit) **selecciona la palabra completa** bajo el cursor. La **rueda de scroll no se ve afectada**: llega como su propio tipo de evento (`SCROLL_UP`/`SCROLL_DOWN`) que el handler del control pasa como `NotImplemented` al handler del `Window` (`_on_chat_scroll`), que conserva el comportamiento original de scroll. Los press se ignoran con un Float abierto o un run en vuelo (mismo guard de `enter_copy_mode`), y `_mouse_down` garantiza que un ciclo press-drag-release no cruza una salida del modo.

**Modo de copia por teclado (fallback).** `F2` (nueva acción `copy_mode`, remapeable/desligable en `[keys]` como cualquier otra) o `/copy` entran en el modo: el **ancla** se fija en la celda superior izquierda del pane visible y el **cursor** arranca en la inferior derecha, de modo que la página visible entera queda preseleccionada (copiar "lo que veo" es un solo Enter). Flechas mueven el extremo que se extiende (←/→ envuelven entre fin/inicio de línea; ↑/↓ conservan la columna), `Home`/`End` saltan al inicio/fin de la fila, `PgUp`/`PgDn` saltan una página completa. `Enter` (o `Ctrl+Y`) **yanka** el rango al clipboard del sistema — `wl-copy`/`xclip`/`pbcopy`, el inverso del read de Ctrl+V — y muestra los caracteres copiados; sin herramienta de clipboard (p. ej. SSH sin `xclip`) avisa por qué no se copió. `Esc` cancela. La selección (mouse o teclado) se pinta como banda de *reverse video* (`\x1b[7m`) sobre el ANSI cacheado (recalcado solo en el modo activo, sin tocar la caché de bloques) y el footer cambia a las pistas del modo — que además anuncian el atajo de doble-click cuando la selección vino del mouse. **Ruteo de teclas:** al activar el modo se apagan los base-bindings (condición `_active_float is None and not _copy_active`), el foco se mueve al *chat window* (enfocable como `Window`), y un set de keys del app vía `DynamicKeyBindings` (activo solo con `_copy_active`) toma el control de flechas/Enter/Esc/Ctrl+Y — así el buffer del composer ya no puede interceptarlas, y tipar en modo normal sigue intacto.

**Código.** Nuevo helper puro `copy_range.word_bounds` (bordes de la palabra bajo `col`; más grueso que el finder de prompt_toolkit para que un doble-click agarre rutas/URLs/identificadores completos) al módulo puro `copy_range.py` (matemática de rango: `Pos`, `range_text`, `selection_line_span`, `step_page`, `plain_lines`, `apply_reverse_highlight`) y el lado write en `clipboard.py` (`write_clipboard_text` + `clipboard_write_available`/`hint`). **OSC 52 (fallback de clipboard):** la investigación (issue prompt_toolkit #1120, PR #1387 aún sin merge; Neovim `vim/ui/clipboard/osc52.lua`, Emacs `select-a-region`, tmux, nano) muestra que el mecanismo estándar para copiar al portapapeles *sin herramienta local* es la secuencia OSC 52 (`ESC ] 52 ; c ; <base64> ST`), que ejecuta el *terminal local* — por eso funciona por SSH. `clipboard.py` añade `osc52_sequence` (pura, testable), `osc52_supported` (detección best-effort por `TERM_PROGRAM`/`TERM`/`KITTY_WINDOW_ID`/`ALACRITTY_INSTANCE_ID`, conservadora para no afirmar éxito donde la secuencia se descarta, p. ej. VTE/GNOME, Terminal.app, PuTTY) y `write_clipboard_osc52` (escribe al tty controlador; `False` si no hay tty). El yank lo usa como fallback cuando no hay herramienta local, gobernado por la nueva opción `clipboard_osc52` (`"auto"`/`"on"`/`"off"`, `PHOSON_CLIPBOARD_OSC52`). Tests: `test_g3_copy_mode_unit.py` (60, de los cuales 16 para el mouse y 11 para OSC 52: formato de secuencia, resolución on/off/auto, detección por `TERM`/`TERM_PROGRAM`/var de env, write con/sin tty, yank con fallback activo e inactivo) + e2e de teclas reales vía PipeInput (F2 → flechas → Enter copia y sale; F2 → Esc cancela; el tipado no se desvía; SGR mouse — drag copia, rueda sigue scroll).
