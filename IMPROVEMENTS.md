...
**Código.** Nuevo helper puro `copy_range.word_bounds` (bordes de la palabra bajo `col`; más grueso que el finder de prompt_toolkit para que un doble-click agarre rutas/URLs/identificadores completos) al módulo puro `copy_range.py` (matemática de rango: `Pos`, `range_text`, `selection_line_span`, `step_page`, `plain_lines`, `apply_reverse_highlight`) y el lado write en `clipboard.py` (`write_clipboard_text` + `clipboard_write_available`/`hint`). Tests: `test_g3_copy_mode_unit.py` (51, de los cuales 16 nuevos para el mouse: `word_bounds`, la cadena press/drag/release, doble-click, click simple, guardia Float/run, passthrough de scroll, resaltado, y e2e de paquetes SGR reales vía PipeInput — arrastre que copia y rueda que sigue scroll — con el driver en `finally` para que un fallo nunca deje la app colgada).
